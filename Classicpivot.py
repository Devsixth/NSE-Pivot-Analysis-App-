...
def classicpivot():
    ...
    df['CLASSIC_PR1'] = (2 * df['PP'] - df['LOW_PRICE']).round().astype(int)
    df['CLASSIC_PR2'] = (df['PP'] + df['HIGH_PRICE'] - df['LOW_PRICE']).round().astype(int)
    df['CLASSIC_PR3'] = (df['CLASSIC_PR1'] + df['HIGH_PRICE'] - df['LOW_PRICE']).round().astype(int)
    df['CLASSIC_PS1'] = (2 * df['PP'] - df['HIGH_PRICE']).round().astype(int)
    df['CLASSIC_PS2'] = (df['PP'] - (df['HIGH_PRICE'] - df['LOW_PRICE'])).round().astype(int)
    df['CLASSIC_PS3'] = (df['CLASSIC_PS1'] - (df['HIGH_PRICE'] - df['LOW_PRICE'])).round().astype(int)

    # Filter columns for display
    filtered_df = df[['SYMBOL', 'PP', 'CLASSIC_PR1', 'CLASSIC_PR2', 'CLASSIC_PR3',
                      'CLASSIC_PS1', 'CLASSIC_PS2', 'CLASSIC_PS3', 'DATE', 'Rate_Cate',
    ...
    with col1:
        ...
        selected_date = st.slider(
            ...
            key="classic_top_date_slider"  # Unique key for the top slider
        )
    ...
    with col2:
        selected_categories = st.multiselect(
            ...
            key="classic_category_select"  # Unique key for categories
        )
    ...
    with col3:
        ...
        selected_symbols = st.multiselect(
            ...
            key="classic_symbol_select"  # Unique key for symbols
        )
    ...
    with left_column:
        ...
        selected_volatilities = st.multiselect(
            ...
            key="classic_volatility_select"  # Unique key for volatility
        )

        selected_rate_cate = st.multiselect(
            "Choose your Stock Price Range",
            options=filtered_ratecate,
            key="classic_rate_cate_select"  # Unique key for rate categories
        )

        selected_directions = st.multiselect(
            "Choose your Change Direction",
            options=filtered_direction,
            key="classic_directions_select"  # Unique key for directions
        )
